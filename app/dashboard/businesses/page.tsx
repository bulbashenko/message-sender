...
import { Business, getBusinesses, createBusiness, searchBusinesses } from "@/app/api/services/communications";
...
export default function BusinessesPage() {
  ...
  useEffect(() => {
    if (status === "authenticated" && session?.user?.access) {
      console.log("Статус: authenticated. Сессия есть. Пытаемся подгрузить businesses...");
      loadBusinesses();
    } else {
      console.log("Либо статус не authenticated, либо нет session?.user?.access:", session?.user?.access);
    }
  ...
  async function handleSearch() {
    ...
    console.log("searchQuery =", searchQuery, "| searchLocation =", searchLocation);
  ...
  async function handleAddBusiness(e: React.FormEvent) {
    ...
    if (!newBusiness.name || !newBusiness.address || !newBusiness.phone_number) {
      console.log("Не заполнены обязательные поля: name, address, phone_number");
      toast({
        title: "Validation Error",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {businesses.map((business) => (
            <Card key={business.id} className="p-6 shadow-lg bg-white dark:bg-gray-800">
